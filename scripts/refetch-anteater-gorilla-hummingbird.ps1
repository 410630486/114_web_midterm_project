# Re-fetch better photos for anteater, gorilla, hummingbird
$targets = @(
    @{ id='anteater'; extraTerms=@('Giant anteater','Myrmecophaga tridactyla','anteater photo') },
    @{ id='gorilla'; extraTerms=@('Gorilla','Gorilla gorilla','gorilla photo') },
    @{ id='hummingbird'; extraTerms=@('hummingbird','Trochilidae','hummingbird photo') }
)
...
function Is-BadCandidate($page, $ii, $fileUrl) {
    # reject non-photo files and obvious diagrams/maps
