# Re-fetch better photos for armadillo and skunk, prefer JPG photographic files and avoid maps/diagrams/PDFs
$targets = @(
    @{ id='armadillo'; extraTerms=@('Dasypus','armadillo mammal') },
    @{ id='skunk'; extraTerms=@('Mephitidae','skunk photo','skunk animal') }
)
...
function Append-Attr([string]$title, [string]$pageUrl, [string]$fileUrl, [string]$author, [string]$license) {
    $entry = "`n---`n`n$title`n`n- Source page: $pageUrl`n- Direct file URL: $fileUrl`n- Author: $author`n- License: $license`n`nNotes: UPDATED — replaced previously incorrect image; please retain attribution when reusing.`n"
    Add-Content -Path $attrPath -Value $entry
}

function Is-BadCandidate($page, $ii, $fileUrl) {
    # Reject PDFs, SVGs; reject if title or url contains map/range/diagram/graph
    $badExt = @('.pdf','.svg','.tif','.tiff')
    foreach ($ext in $badExt) { if ($fileUrl.ToLower().EndsWith($ext)) { return $true } }
    $lower = ($page.title + ' ' + $fileUrl).ToLower()
    $badKeywords = @('map','range','distribution','genera','diagram','chart','mapa')
...
foreach ($t in $targets) {
    ...
    try { $resp = Invoke-RestMethod -Uri $api -UseBasicParsing -TimeoutSec 30 } catch { Write-Output ("API error for {0}: {1}" -f $term, $_); continue }
        if ($resp.query -and $resp.query.pages) {
            # sort pages: prefer JPEG by url extension and larger images (extmetadata.ImageWidth)
            $candidates = @()
            ...
            # Order: jpg first, then width desc
